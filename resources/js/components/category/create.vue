        ...
        <el-row>
            <el-col :span="24">
                <el-card shadow="never">
                    <div slot="header">
                        <span>{{$route.params.id ? 'Edit' : 'Create'}} Category</span>
                        </div>
...
<script>
    export default {
        ...
        beforeRouteEnter (to, from, next) {
            if (to.params.id) {
                axios.get(`/api/categories/${to.params.id}`).then(function (response) {
        ...
        methods: {
            ...
            onCancel(){
                this.$router.push({name: 'view-category'})
            },
            onSubmit() {
                var vm = this
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        axios[id ? 'put' : 'post'](`/api/categories${id ? `/${id}` : ''}`, vm.form).then(function () {
                            vm.disabled = false
                            vm.$message({
                                type: 'success',
